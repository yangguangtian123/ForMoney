...
- (void)createLeftNavItem{
    UIBarButtonItem *leftItem = [[UIBarButtonItem alloc]initWithTitle:@"取消" style:UIBarButtonItemStylePlain target:self action:@selector(backPerPage)];//返回上一个界面
    leftItem.tintColor = Color(73, 130, 229,1.0);
...
- (void)passSelectedAssetsToDelegate
{
    ...
    for (NSURL *selectedAssetURL in self.selectedAssetURLs) {
        ...
        NSLog(@"dddddddddd %@",selectedAssetURL);
                                    ...
                                    [self.assetsLibrary enumerateGroupsWithTypes:ALAssetsGroupPhotoStream usingBlock:^(ALAssetsGroup *group, BOOL *stop)
                                     {
                                         ...
                                         NSLog(@"Cann't found asset from stream:%@",[error localizedDescription]);
                                     }];
